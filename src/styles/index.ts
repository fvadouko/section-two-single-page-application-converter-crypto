const styles = {
  // App.js
  container:
    'flex justify-center min-h-screen sm:px-16 px-6 bg-black',
  innerContainer:
    'flex justify-between items-center flex-col max-w-[1280px] w-full',
  header:
    'flex flex-row justify-between items-center w-full sm:py-10 py-6',
  exchangeContainer:
    'flex-1 flex justify-start items-center flex-col w-full mt-10',
  headTitle:
    'text-white font-poppins font-black text-5xl tracking-wide',
  ...
  exchange:
    'w-full min-h-[400px] bg-black backdrop-blur-[4px] rounded-3xl shadow-card flex p-10',
  ...
  currencyList:
    'absolute z-10 right-0 bg-black border-[1px] border-site-dim2 w-full mt-2 rounded-lg min-w-[170px] overflow-hidden -top-[10em] -left-[10em]',
  currencyListItem:
    'font-poppins font-medium text-base text-white hover:text-dim-white px-5 py-3 flex hover:bg-site-dim2 cursor-pointer',
  ...
  // WalletButton
  walletButton:
    'hover:bg-pink-400 border-none outline-none px-6 py-2 font-poppins font-bold text-lg text-white rounded-3xl leading-[24px] bg-pink-600 transition-all',

  // loader
  loader:
    'flex justify-center items-center flex-col w-full min-h-full',
